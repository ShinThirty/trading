...
Note: ``symbol`` is the traded/watched **futures** symbol (e.g. ``/MES``) — the
same instrument the detector watches and the paper broker fills, so ``levels`` are
prices on that instrument itself. A level with a ``direction`` (``long``/``short``)
is tradeable: when it tags, the paper bracket opens in that direction with an
**absolute-price** OCO — ``stop`` (the bail price, now load-bearing) and ``target``
(the take-profit), each falling back to the plan's ``default_stop_points`` /
``default_target_points`` offset from the level when omitted. A level with no
``direction`` is alert-only (notify, no paper trade). Each level also carries a
``mode`` — ``fade`` (touch-and-reject, the positive-GEX setup) or ``break``
(cross-and-follow-through, the negative-GEX setup) — which the detector uses to pick
its trigger geometry.
...
class Level:
    ...
    # reversal/retest are a wall's two verdict rows — same price+side, different direction —
    # driven by a BreakoutTracker (geometry, not tape); see detector.py / breakout.py.
    mode: str = "fade"
    direction: str | None = None  # "long" | "short"; None = alert-only (notify, no paper trade)
    stop: float | None = None  # explicit bail price; None → derive from default_stop_points
    # explicit take-profit price; None → derive from default_target_points
...
class SessionPlan:
    date: str
    symbol: str  # the traded/watched futures symbol (e.g. "/MES")
    regime: str
    ...
    default_stop_points: float = 6.0  # child stop = level ∓ points (side per direction)
    default_target_points: float = 8.0  # child take-profit = level ± points
...
def load_session_plan(path: Path) -> SessionPlan:
    ...
    levels = [
        Level(
            ...
            direction=(str(lvl["direction"]) if lvl.get("direction") else None),
            stop=(float(lvl["stop"]) if lvl.get("stop") is not None else None),
            target=(float(lvl["target"]) if lvl.get("target") is not None else None),
        )
    ...
    return SessionPlan(
        ...
        levels=levels,
        default_stop_points=float(data.get("default_stop_points", 6.0)),
        default_target_points=float(data.get("default_target_points", 8.0)),
