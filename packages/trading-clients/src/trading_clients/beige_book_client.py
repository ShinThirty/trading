"""Federal Reserve Beige Book HTTP transport (no auth, identifies via User-Agent).

The Fed's beige-book pages are static HTML — no Playwright needed. Index page
gets a short TTL (refreshed on release days); individual summaries are
immutable once published and cached for 30 days.
"""
...
class BeigeBookClient(BaseClient):
