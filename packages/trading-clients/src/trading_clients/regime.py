...
def classify_breadth(
    ...
    # Signal 4: SPY vs RSP equal-weight concentration (optional)
    if rsp_closes is not None and len(rsp_closes) >= lookback + 1:
        ...
        spy_rsp_div = spy_ret - rsp_ret
        if spy_rsp_div > 2.5:
            parts.append(
                f"megacap concentration SPY beating RSP by {spy_rsp_div:.1f}pp ({lookback}d)"
            )
            warnings += 1
        elif spy_rsp_div < -1.5:
            parts.append(f"broadening RSP leading SPY by {abs(spy_rsp_div):.1f}pp ({lookback}d)")
            strengths += 1
        else:
            parts.append(f"SPY/RSP aligned ({spy_rsp_div:+.1f}pp)")
...
# ═══════════════════════════════════════════════════════════════
# Bear regime score — composite 0-10 across 8 dimensions
# ═══════════════════════════════════════════════════════════════
#
