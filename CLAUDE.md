...
| **TastyTrade** | IV rank/percentile, backtesting, watchlists, dividends; read-only account (accounts, balances, positions, orders) folded into the portfolio aggregation; **DXLink (dxFeed) futures streaming** — the scalper's /MES feed via `get_quote_token` + `dxlink_stream_client.py` (needs a funded customer account for CME data) | OAuth2 refresh token |
