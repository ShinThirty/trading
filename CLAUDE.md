...
4. **trading-scalper** — Local **paper** entry-detector for intraday **index-futures (/MES)** scalps (see [docs/scalper-automation.md](docs/scalper-automation.md)). **Migrated 2026-07-12** from SPY/QQQ 0DTE options to /MES (Micro E-mini S&P 500) — linear $/pt P&L, no theta decay while waiting for a setup, free two-sided direction (a real short, not a long put), and one instrument that *is* the level map (no OCC layer). (This followed the 2026-06-12 pivot from the old assist-only stop-enforcer once Webull's native bracket made the auto-stop redundant.) Job: (1) **prompt** when the future triggers a blessed session-plan level — a *confirmed* setup, not a bare touch: `SetupDetector` + `Notifier` are **GEX-regime-aware**, branching on each level's `mode` (`fade` = touch-and-reject at a gamma wall in +GEX; `break` = cross-with-follow-through in −GEX), gated on lean (by trade *direction*) and on the tape agreeing with the trade direction, with a spread gate on the paper fill. Two further modes render the verdict a bare `break` can't — `reversal` (a *failed* break: cross-out that snaps back inside → trade the snap-back, opposite the attempt) and `retest` (a *confirmed* break that pulls back to the wall and resumes → the patient continuation) — driven by a per-wall `BreakoutTracker` (`breakout.py`) and gated on **geometry + lean only, never tape** (a wall = a `reversal` row + a `retest` row at one price+side; the tracker is level-source-agnostic so ORB/VWAP/stop-cluster sources plug in later). A `break` level **arms** only after the detector has *witnessed* price on the pre-break side, and a stream-gap guard (`gap_s`) drops all arming (and the verdict trackers' cross-timers) on a suspend/disconnect — so a daemon that slept through the actual cross can't wake up and fire at the extended top (downtime-safe break entries). Geometry constants are **rescaled ~10×** for the /ES-family price magnitude (~6250 vs QQQ ~600), pending recalibration against recorded /MES tape (a fresh version cohort insulates the QQQ history). The plan's walls + zero-gamma flip come from `get_gamma_profile` on **SPX cash-index options directly** (the deepest dealer-gamma pool and true driver of the index the future tracks — ETFs like SPY/QQQ are out, their index gap is dividend/tracking noise, not clean carry), then shifted to /MES prices by the **live carry basis** (`/MES − SPX`, pure cost-of-carry; measured off DXLink and recorded as a shadow series by `basis.py`, and printed for prep by `trading-scalper --basis`) — **not** a ×10 of SPY. The traded contract is **exchange-verified, not computed**: `contract.py::resolve_contract` calls Tastytrade `GET /instruments/futures?product-code[]=MES` at startup, so a bare root (`--symbols /MES`) resolves to the exchange's own `active-month` contract and the broker's $/pt + tick come from `notional-multiplier` / `tick-size`. All three — which month is live, what a point pays, what a tick is — are **absent from the repo**; `instruments.py` is now a `Profile` registry holding only what the venue has no opinion on (the detector's `Geometry` bands = our calibration, and the cash-index `reference` = our level-source decision), so adding a root means answering just those two questions. A *dated* symbol is honored but checked (warns if it's no longer the front month, or is closing-only). A lookup the exchange can't answer is a **hard stop** (`ContractError` → no session): no date-math fallback, no hardcoded multiplier, because both fail silently — a stale contract still streams, still fires, still fills on a dead book, and a wrong multiplier leaves every P&L number plausible and wrong. (2) on every confirmed prompt **auto-execute the same direction-aware bracket** — entry (BUY long / SELL short) + OCO stop-loss/take-profit at **absolute prices** (plan points, tick-rounded), not a premium % — in an in-memory `PaperBroker.place_bracket`; (3) **persist** fills + a realized-P&L summary (`PaperPersister` → `~/.trading/scalp/paper/{date}.jsonl` + `-summary.json`) so the detector's track record is reviewable, stamping each fire row with the detector `version` (`version.py`) as the grading cohort key (the futures migration opens cohort **0.4**). `scorecard.py` (the `trading-scalper-scorecard` script) then grades that track record **per version cohort × verdict mode** — n / win rate / expectancy + its 95% lower bound / profit factor / max drawdown / session concentration — against a go-live gate evaluated on the current cohort only (n≥50 over ≥10 sessions, PF≥1.5, expectancy LB>0, concentration<40%); a mode passes → git-tag it → the future live runner deploys from the tag. Paper-only — there is no live-order path; the safety story is zero real capital, and the ledger models a real short (uncapped, unlike the options book's capped-loss long put). Two ports: `MarketDataFeed` (**Tastytrade DXLink / dxFeed** streaming — `dxlink_stream_client.py`) + `BrokerExecution` (`PaperBroker`, multiplier + tick per the resolved contract). The session plan is a dated, hand-editable YAML naming a **`direction`** (`long`/`short`) + `mode` (fade/break/reversal/retest) + optional explicit `stop`/`target` price per level; `symbol` is the dxFeed streamer symbol (e.g. `/MESU26:XCME`). Levels/lean/direction hot-reload; a new symbol mid-session needs a restart. `cli.build_session` is the composition root behind the `trading-scalper` entry point; `--demo-setup SYMBOL[:QTY[:PRICE]]` runs the loop offline. Roadmap: DXLink greeks feed; autonomous live entry only after a verdict mode clears the scorecard's go-live gate. Depends on trading-clients[streaming] + pyyaml.
...
│   │           ├── tastytrade.py        # 11 endpoints (IV metrics, backtesting, watchlists, dividends, futures instruments + read-only account: accounts, balances, positions, orders)
...
| **Tradier** | Option chains, greeks, IV, quotes; read-only account (profile, balances, positions, orders) folded into the portfolio aggregation; **equity streaming market data** (top-of-book quote + trade + timesale via WebSocket) — production-only (no longer the scalper feed after the 2026-07-12 futures migration; the scalper now streams /MES via Tastytrade DXLink) | Bearer token |
