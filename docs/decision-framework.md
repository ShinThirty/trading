...
**Issuer credit:** Where a thesis turns on financing — a debt-funded buildout, a refinancing wall, a covenant test, capex against negative FCF — run `get_issuer_credit`. It prices the company's own bonds off ETF holdings files and reports their spread to Treasuries against rating-cohort benchmarks, which is a read the equity tape doesn't carry and which usually moves first. It is also the valuation-disconnect measure that works when analyst targets don't (see [bearish-framework.md](bearish-framework.md)). Output is T+1 evaluated marks, YTM not YTW, and nominal G-spreads not OAS — compare placement against a cohort, not basis points. No priceable bonds (an all-convertible capital structure) means **n/a**, not "no disconnect".
