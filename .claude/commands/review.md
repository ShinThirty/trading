...
2bb. **Single-name credit on the financing-dependent cluster.** Item 2b covers the *market-wide* private-credit channel; this is the issuer-level read that was manual until now. Call `get_issuer_credit` on the names whose thesis rests on external funding — the [[project_circular_financing_buckets]] cluster and any current [[project_neocloud_financing_tripwires]] watch names (CRWV, APLD; ORCL for the standing G-spread item in [[project_ai_capex_funding_phase]]).

   **Record the median G-spread per name in retro.** The level is context; the *change since last review* is the signal, and it only exists if the prior number was written down. What to look for, in order:
   - **Spread widening while the equity is flat-to-higher** — the two markets disagreeing. This is the valuation-disconnect row in [bearish-framework.md](../../docs/bearish-framework.md).
   - **Ratio to the matched rating cohort** — ≥2x means credit prices the name materially below its rating. Read placement, not basis points: these are nominal G-spreads on callable paper against option-adjusted index spreads, so the ratio runs high by construction.
   - **Cohort dispersion** — CRWV at ~700bp against APLD at ~324bp (8/14) says the stress is issuer-specific, not sector-wide. A cohort converging *wider* together is a different and more serious signal than one name being an outlier.

   Names with no priceable bonds (IREN, NBIS — debt is entirely convertibles) score **n/a**, not "fine". Also cross-read against the *terms* of any new issuance since the last review, per [[project_ai_capex_funding_phase]]: a deal closing is not health, the spread it closed at is the datum.
...
5. **Income Sleeve ledger** — the harvest-premium theta book, reviewed as ONE ledger and kept separate from the growth-first CSPs in items 1–3. Per [income-sleeve.md](../../docs/income-sleeve.md), the sleeve spans the tax-advantaged accounts (Webull Roth IRA + Fidelity BrokerageLink / BrokerageLink Roth / HSA); the main Webull Individual Cash/Margin accounts are the growth book (Section 2). Call **`get_income_sleeve_ledger`** — it reads the transaction feed (contribution-immune) and returns net premium, buyback debits, share-leg cash flow, current collateral, and the annualized premium-yield vs the ~4.5% floor since the 2026-07-18 inception. Then read:
   - **Net premium vs T-bill floor** (the mandate metric) — is the premium-yield clearing T-bills *meaningfully* net of tested weeks? If not, flag for kill/resize per the doc. (Tool covers the Fidelity sleeve accounts; add the Webull Roth leg manually until its feed lands.)
   - **Cap utilization** — collateral vs the **$250K cap**; cross-check the 60% rule via `get_csp_utilization` (sleeve + growth-first entry-CSP collateral ≤ 60% of cash).
