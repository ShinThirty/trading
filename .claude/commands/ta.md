...
1. **Drawdown.** From `get_entry_signals` (drawdown_pct) and `get_conviction_metrics` if invoked. Surface: % off 52w high, % off recent peak (if recent peak < 52w high), % off entry/cost basis if a position exists (skip otherwise).
   ...
   **When `get_entry_signals` returns a "2W Net (since gap)" row, quote both numbers.** The 2W Net is endpoint-to-endpoint and cannot see a gap inside the window — a break plus a recovery nets to ~flat and reads as "no move" while the tape actually ripped. The since-gap figure is the path. This also means the Front-Run Catalyst / Pre-Priced Selloff breakers can stay silent on a genuinely violent two weeks; check the pair before concluding the tape was quiet.

2. **Trend classification + strength.** Use the **Tape Pattern tag** returned by `get_entry_signals` directly — *Rally → Gap-Down, Selloff → Gap-Up, Gap-Down → Recovery, Gap-Down → Basing, Gap-Up → Hold, Gap-Up → Fade, Steady Rally, Steady Decline, Choppy, Quiet, Gap-Up, Gap-Down, Drift*. Do not re-derive. A `Gap-* → *` tag means one event several sessions back drove the window — read the note's reclaim % as the real story and do **not** treat the wide range as two-way chop. Pair it with: current price vs SMA20 / SMA50 / SMA200 (from `get_technical_indicators`), and weekly vs daily MA alignment (price > 50w SMA = weekly uptrend). **Quantify strength with ADX(14)**: <20 ranging (mean-revert / harvest-premium structures fit), 20-25 transitional, >25 trending (don't fade, don't average down against +DI/-DI dominance), >40 strong trend. ADX direction (+DI vs -DI) confirms the Tape Pattern label.
