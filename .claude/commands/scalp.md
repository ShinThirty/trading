...
Intraday /MES futures scalp workflow, mode **$ARGUMENTS.mode** (default: pre-session prep). The traded instrument is **/MES** (Micro E-mini S&P 500, the paper daemon's instrument); the gamma walls come from **SPX** cash-index options (`get_gamma_profile SPX` — the true dealer-gamma driver, no ETF tracking noise), already in index points (~7575), and are shifted to /MES prices by the **live carry basis** (`/MES − SPX`, ≈ +50 in RTH), fetched with `trading-scalper --basis`. No SPY, no ×10.
...
3. **Analyze SPX, trade /MES.** The gamma walls come from **SPX** cash-index options (`get_gamma_profile SPX`) — the deepest, cleanest dealer-gamma pool and the true driver of the index /MES tracks; **ETFs (SPY/QQQ) are out** (their index gap is dividend/tracking noise, not clean carry). SPX walls are index points (~7575); shift them to /MES by the **live carry basis** (`/MES − SPX`, pure cost-of-carry, ≈ +50 in RTH) from `trading-scalper --basis`. **Not ×10.** Never pull /MES quotes/chains/gamma from the equity tools — they won't resolve; the daemon streams /MES itself. The daemon watches + paper-trades /MES off the basis-shifted levels.
...
**Step 3 — level map.** Call `get_quote SPX` (prev close, day H/L) + `get_timesales SPX` (prior/current session, 15min) for the static pivots, expressed in **SPX index points then basis-shifted to /MES** like the walls. Map and state as **price points**:
- Prior-day high / low / close (the primary pivots).
- Round numbers inside the range (these are the same on the /MES scale — 7600 is 7600).
...
For deeper structural S/R, defer to `/ta SPX` rather than re-deriving — reuse, don't duplicate.
...
1. **Pick the resolution(s) per wall, then assign a direction to each.** The walls from Step 1 are the edges; *which resolution(s) you trade* at each edge is decided here — by the regime sign **and the wall's character**. A wall has three physical resolutions (touch-and-reject, cross-and-fail, cross-and-hold); the four modes express them. **The daemon never picks a mode — it executes whatever you write here, and can't recompute GEX intraday — so this is the only place the verdict modes get deployed. Don't default trend days to bare `break`; route to the tree:**
   ...
   **No contract to pick — the future *is* the instrument.** Add the live carry basis to each SPX gamma wall to get its /MES price (SPX 7600 + basis 50 → /MES 7650) and set the level `price` to the /MES value; fetch the basis with `trading-scalper --basis` (RTH only — SPX must be live). There is no expiry / strike / OCC step — that whole options layer is gone. Optionally set explicit `stop`/`target` /MES prices per level; otherwise the plan's `default_stop_points` / `default_target_points` derive them from the level.
...
3. **State these caveats after writing — they are easy to get wrong:**
   ...
   - **SPX wall + carry basis → /MES.** Every level `price`, `stop`, `target`, and `zero_gamma` is a /MES price (~7600), i.e. the SPX gamma wall + the live basis (`trading-scalper --basis`). A wall left at SPX scale (~7575) tags ~50 pts early — the /MES tape trades above cash by the carry. Re-check the basis each session (it decays into expiry and jumps at the quarterly roll).
   ...
   - **Run it:** `uv run --package trading-scalper trading-scalper --symbols /MESU26:XCME` (the plan's `symbol`). Needs `[tastytrade]` in `~/.tradingrc` (DXLink feed) + a funded customer account for CME data. It also streams **SPX** and records the **live carry basis** (`{date}-basis.jsonl`, shadow — gating nothing; valid RTH only, when SPX is live) plus shadow volume telemetry (`{date}-tape.jsonl` raw /MES tape + `{date}-shadow.jsonl` POC/value-area + volume-rate snapshots). Disable the basis with `--reference ''`. After the close, `/scalp review` grades the daemon's paper record (`{date}.jsonl` + `{date}-signals.jsonl` + `{date}-summary.json`) and the shadow observers.
...
Get the live **/MES** price directly with `trading-scalper --basis` — it prints the front-month /MES price (the actual tradeable number, no proxy) and the current carry basis. Add `get_quote SPX` for cash context. Compare the /MES price straight against the /MES level map. For /MES VWAP / intraday structure, read the running daemon's shadow snapshot (`~/.trading/scalp/paper/{date}-shadow.jsonl`, latest POC/value-area) or the live /MES chart — there's no MCP intraday feed for the future. Output 3 lines max:
